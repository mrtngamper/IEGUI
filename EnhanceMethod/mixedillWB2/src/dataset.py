...
class Data(Dataset):
  def __init__(self, imgfiles, patch_size=128, patch_number=32, aug=True,
    ...
    if self.mode == 'testing':
      ...
      self.deepWB_T.eval().to( device = torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
      self.deepWB_S.eval().to( device = torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
    ...
    if self.mode == 'training':
      ...
      if checks:
        ...
        t_img, s_img = deep_wb(d_img, task='editing', net_s=self.deepWB_S,
                               net_t=self.deepWB_T,  device = torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
        if self.keep_aspect_ratio:
          d_img = ops.aspect_ratio_imresize(d_img, max_output=t_size)
          t_img = ops.aspect_ratio_imresize(t_img, max_output=t_size)
          s_img = ops.aspect_ratio_imresize(s_img, max_output=t_size)
  ...
  @staticmethod
  def load_files(img_dir, mode='testing'):
